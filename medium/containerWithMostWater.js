...
function checkWater2Pointer(arr) {
  ...
  while (i < j) {
    ...
    //move the increment with the smallest height
    //intuition: bigest volume comes from longer the distance, and taller the height
    //as we reduce the search space, we want to keep the tallest possible height
    arr[i] < arr[j] ? i++ : j++;
  }
